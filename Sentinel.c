...
#define MAX 10
int SentinelSearch(int [],int,int);	/* 	arguments are array to hold the elements, no. of elements in the array, key element to search for
...
{
    arr[n]=key;                     //assigning the sentinel
    for(int i=0;i<=n;i++)
    {
        if (arr[i]==key)
            return i+1;
